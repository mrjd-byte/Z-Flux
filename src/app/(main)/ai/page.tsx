...
export default function AIDecisionEngine() {
  ...
  const getRecommendations = () => {
    ...
    if (data.savingsRate < 0.2 && data.income > 0) {
      recs.push({
        ...
        why: "Increasing savings by 5% would improve your Vitality Index by ~4 points based on current weights."
      });
    }
  ...
  const applyPlan = async () => {
    ...
    try {
      ...
      if (res.ok) {
        alert("Smart Plan applied! Your budgets have been updated.");
        fetchAIData();
      }
    } catch (e) {
      console.error(e);
      ...
      {activeTab === "recommendations" && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {getRecommendations().map((rec, i) => (
            <GlassCard key={i} className={`p-8 border-l-4 ${
              rec.type === 'danger' ? 'border-rose-500' : rec.type === 'warning' ? 'border-amber-500' : 'border-emerald-500'
            }`}>
              <div className="flex justify-between items-start mb-4">
                <h3 className="text-lg font-bold text-white tracking-tight">{rec.title}</h3>
                <div className={`p-2 rounded-lg ${
                  rec.type === 'danger' ? 'bg-rose-500/10 text-rose-400' : rec.type === 'warning' ? 'bg-amber-500/10 text-amber-400' : 'bg-emerald-500/10 text-emerald-400'
                }`}>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <GlassCard className="p-10 relative overflow-hidden">
               ...
               <div className="flex items-baseline gap-4">
                  ...
                  <span className="text-zinc-500 uppercase text-[10px] font-black tracking-widest">Vitality Units</span>
               </div>
